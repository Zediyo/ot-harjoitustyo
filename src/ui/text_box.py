...
class TextBox:
    ...
    def draw(self, display):
        ...
        if self._active:
            pygame.draw.rect(display, self._border_color, self._rect, border_radius=5, width=2)

        rendered_text = self._font.render(text[self._overflow_offset:], True, color)
        display.blit(rendered_text, (self._rect.x + 5, self._rect.y + 5))

        if self._error_text:
            error_surface = self._font.render(self._error_text, True, (255, 0, 0))
            display.blit(error_surface, (self._rect.x + 5, self._rect.y + self._rect.height + 5))
    ...
    def _update_overflow_offset(self):
        ...
        text_width = self._font.render(self._text[self._overflow_offset:], True, self._text_color).get_width()
