...
class TestLevelEditor(unittest.TestCase):
    def setUp(self):
        ...
        self.editor = LevelEditor({"id": "1", "name": "potato", "data": self.data})

        data = self.editor._map.data

        self.spawn_location = [(x, y) for y in range(len(data)) for x in range(len(data[0])) if data[y][x] == constants.TILE_SPAWN]
        self.end_location = [(x, y) for y in range(len(data)) for x in range(len(data[0])) if data[y][x] == constants.TILE_END]
        self.spawn_screen_location = [(x * constants.TILE_SIZE, y * constants.TILE_SIZE) for x, y in self.spawn_location]
    ...
    def test_init(self):
        self.assertEqual(self.editor._has_required, {"spawn": True, "end": True })

    def test_input_remove_and_add_end(self):
        # cant add end (already exists)
        self.assertEqual(self.editor._has_required, {"spawn": True, "end": True }) 
        self.editor.input_raw([pygame.event.Event(pygame.KEYDOWN, key=pygame.K_5)])
        self.editor.input_mouse("left", (4 * constants.TILE_SIZE, 4 * constants.TILE_SIZE))
        self.assertNotEqual(self.editor._map.get_cell(4, 4), constants.TILE_END)

        self.editor.input_mouse("right", self.end_screen_location[0])
        self.assertEqual(self.editor._has_required, {"spawn": True, "end": False })
    ...
    def test_input_remove_and_add_spawn(self):
        ...
        self.assertEqual(self.editor._has_required, {"spawn": True, "end": True }) 
        self.editor.input_raw([pygame.event.Event(pygame.KEYDOWN, key=pygame.K_4)])
        self.editor.input_mouse("left", (4 * constants.TILE_SIZE, 4 * constants.TILE_SIZE))
        self.assertNotEqual(self.editor._map.get_cell(4, 4), constants.TILE_SPAWN)

        self.editor.input_mouse("right", self.spawn_screen_location[0])
        self.assertEqual(self.editor._has_required, {"spawn": False, "end": True })
    ...
    def test_input_add_and_remove_enemy(self):    
        self.editor.input_raw([pygame.event.Event(pygame.KEYDOWN, key=pygame.K_3)])
        self.editor.input_mouse("left", (4 * constants.TILE_SIZE, 4 * constants.TILE_SIZE))
        self.assertEqual(self.editor._map.get_cell(4, 4), constants.TILE_ENEMY)

        self.editor.input_mouse("right", (4 * constants.TILE_SIZE, 4 * constants.TILE_SIZE))
        self.assertEqual(self.editor._map.get_cell(4, 4), 0)

    def test_input_add_and_remove_placeable(self):    
        self.editor.input_raw([pygame.event.Event(pygame.KEYDOWN, key=pygame.K_2)])
        self.editor.input_mouse("left", (4 * constants.TILE_SIZE, 4 * constants.TILE_SIZE))
        self.assertEqual(self.editor._map.get_cell(4, 4), constants.TILE_PLACEABLE)
    ...
    def test_input_add_and_remove_block(self):
        ...
        self.editor.input_raw([pygame.event.Event(pygame.KEYDOWN, key=pygame.K_1)])
